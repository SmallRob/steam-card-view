...
(function () {
    ...
    function extractGamesFromReactQuery() {
        try {
            const rootEl = document.querySelector('[data-react-nav-root]') || document.getElementById('CommunityTemplate');
    ...
    /**
     * 解析Steam时间文本为分钟数
     * 支持格式: "1,234.5hrs on record", "123.4hrs", "45.6hrs /2weeks",
     * "0.1hrs", "1,234小时", "123.4h" 等
     */
    function parsePlaytimeToMinutes(text) {
        if (!text) return 0;
        text = text.trim().toLowerCase();
        // 英文格式: "1,234.5hrs on record" / "123.4hrs" / "0.1hrs"
        let match = text.match(/([\d,.]+)\s*hrs?/);
        if (match) {
            const hours = parseFloat(match[1].replace(/,/g, ''));
            return Math.round(hours * 60);
        }
        // 中文格式: "1,234.5小时" / "123.4小时"
    ...
    function extractGamesFromDOM() {
        ...
        rows.forEach(row => {
            try {
                ...
                if (!appid) {
                    const linkEl = row.querySelector('a[href*="/app/"]') || row.querySelector('a[class*="_5rP-WhERE5Q"]');
                ...
                const nameEl = row.querySelector('[class*="game_name"]') || row.querySelector('.game_list_item_name');
                ...
                // 提取游玩时间
                const playtimeEl = row.querySelector('[class*="ANL1vYNAS6E"]') || row.querySelector('[class*="playtime"]') || row.querySelector('.game_list_playtime');
                const playtimeText = playtimeEl?.textContent?.trim() || '';
                // 提取最后游玩时间
                const lastPlayedEl = row.querySelector('[class*="_09Z65-SltXY"]') || row.querySelector('[class*="last_played"]') || row.querySelector('.game_list_last_played');
    ...
    function findScrollContainer() {
        ...
        for (const sel of knownSelectors) {
            ...
            if (el) {
                ...
                if (style.overflowY === 'auto' || style.overflowY === 'scroll' || el.scrollHeight > el.clientHeight + 50) {
                    console.log('[Steam卡片] 找到已知滚动容器:', sel);
                    return el;
                }
            }
        }
        ...
        for (const el of allEls) {
            if (el === document.body || el === document.documentElement || el.id === 'scv-root') continue;
            const style = window.getComputedStyle(el);
            if ((style.overflowY === 'auto' || style.overflowY === 'scroll') && el.scrollHeight > el.clientHeight + 100 && el.clientHeight > 200 && el.scrollHeight > bestHeight) {
                bestHeight = el.scrollHeight;
                best = el;
            }
        }
        if (best) {
            console.log('[Steam卡片] 通用方案找到滚动容器:', best.tagName, best.className.slice(0, 60));
            return best;
        }
        console.log('[Steam卡片] 未找到专用滚动容器, 回退到window');
        return null;
    }
    ...
    function autoLoadAllGamesFromDOM() {
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                ...
                const maxScrollAttempts = Math.ceil(CONFIG.AUTO_LOAD_MAX_WAIT / 300);
                const scrollStep = () => {
                    if (autoLoadAbort) {
                        isAutoLoading = false;
                        resolve(false);
                    ...
                    setTimeout(() => {
                        const currentCount = document.querySelectorAll('[class*="JeLbcWPaZDg"], [class*="game_list_row"], [data-appid]').length;
    ...
    async function fetchAllAchievements() {
        ...
        showLoadingOverlay();
        updateLoadingProgress(`准备获取成就数据...`);
    ...
    function injectStyles() {
        GM_addStyle(`
            ...
            .scv-search input {
                ...
                font-size: 14px;
                outline: none;
                transition: border-color var(--scv-transition);
                box-sizing: border-box;
            }
            .scv-search input:focus {
                border-color: var(--scv-accent);
            }
            .scv-search input::placeholder {
                color: var(--scv-text-secondary);
            }
            .scv-search-icon {
                position: absolute;
                left: 10px;
            ...
            .scv-sort select {
                ...
                background-image: url("data:image/svg+xml,%3Csvg width='12' height='12' viewBox='0 0 12 12'%3E%3Cpath fill='%238f98a0' d='M6 8L1 3h10z'/%3E%3C/svg%3E");
            }
            .scv-sort select:focus {
                border-color: var(--scv-accent);
            }
            ...
            .scv-btn {
                ...
                border: 1px solid var(--scv-border);
                border-radius: var(--scv-radius);
                background: var(--scv-bg-secondary);
                color: var(--scv-text-primary);
                font-size: 13px;
                cursor: pointer;
                transition: all var(--scv-transition);
                white-space: nowrap;
                user-select: none;
            ...
            .scv-btn:hover {
                background: var(--scv-bg-card-hover);
                border-color: var(--scv-accent);
            ...
            .scv-btn--accent:hover {
                background: linear-gradient(135deg, #88d5ff 0%, #66c0f4 100%);
                color: #fff;
            }
            .scv-btn--success {
                background: linear-gradient(135deg, #5c7e10 0%, #4a6b0d 100%);
                color: #d2efa0;
                border: 1px solid #6b8a1a;
            }
            .scv-btn--success:hover {
                background: linear-gradient(135deg, #6b8a1a 0%, #5c7e10 100%);
                color: #e5ff8a;
            }
            .scv-btn--csv {
                background: linear-gradient(135deg, #8b5e3c 0%, #6b4423 100%);
                color: #f0d9b5;
                border: 1px solid #a0724d;
            }
            .scv-btn--csv:hover {
                background: linear-gradient(135deg, #a0724d 0%, #8b5e3c 100%);
                color: #ffe0b2;
            }
            .scv-btn--achievement {
                background: linear-gradient(135deg, #c7a02e 0%, #a68520 100%);
                color: #fff5cc;
                border: 1px solid #d4b44a;
            }
            .scv-btn--achievement:hover {
                background: linear-gradient(135deg, #d4b44a 0%, #c7a02e 100%);
                color: #fffde0;
            }
            .scv-btn--reload {
                background: linear-gradient(135deg, #4a6b8a 0%, #3a5570 100%);
                color: #b8d4e8;
                border: 1px solid #5a7b9a;
            }
            .scv-btn--reload:hover {
                background: linear-gradient(135deg, #5a7b9a 0%, #4a6b8a 100%);
                color: #d0e8f8;
            }
            ...
            .scv-card:hover {
                ...
                box-shadow: var(--scv-shadow-hover);
            ...
            .scv-card:hover .scv-card-img-wrap img {
            ...
            .scv-card-achievement-badge svg {
                width: 12px;
                height: 12px;
                fill: #c7a02e;
                flex-shrink: 0;
            ...
            .scv-card-name {
                ...
                white-space: nowrap;
                transition: color var(--scv-transition);
            }
            .scv-card:hover .scv-card-name {
                color: var(--scv-text-highlight);
            }
            .scv-card-meta {
                display: flex;
            ...
            .scv-card-achievement-bar {
                height: 4px;
                border-radius: 2px;
                background: rgba(199,160,46,0.15);
                overflow: hidden;
                margin-top: auto;
            }
            ...
            .scv-toast {
                ...
                padding: 12px 20px;
                border-radius: var(--scv-radius);
                border: 1px solid var(--scv-accent);
                box-shadow: 0 4px 16px rgba(0,0,0,0.5);
                font-size: 14px;
                z-index: 99999;
                animation: scv-toast-in 0.3s ease;
                transition: opacity 0.3s ease;
            }
            .scv-toast--success {
                border-color: #5c7e10;
            }
            @keyframes scv-toast-in {
                from { transform: translateY(20px); opacity: 0; }
                to { transform: translateY(0); opacity: 1; }
            }

            .scv-card-enter {
                animation: scv-card-fade-in 0.35s ease both;
            }
            @keyframes scv-card-fade-in {
        ...
        toolbar.innerHTML = `
            <div class="scv-toolbar-left">
                ...
                <div class="scv-sort">
                    <select id="scv-sort-select">
                        ...
                        <option value="appid">按AppID</option>
                    </select>
                </div>
            </div>
            <div class="scv-toolbar-right">
                <button class="scv-btn scv-btn--achievement" id="scv-fetch-achievements" title="获取所有游戏的成就数据(需要Steam个人资料公开)">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 15l-2-5 9-11h-5l2-5-7 5z"/></svg>
                    获取成就
                </button>
    ...
    function createCard(game, maxPlaytime, index) {
        ...
        if (achTotal > 0) {
            const achPct = ((achUnlocked / achTotal) * 100).toFixed(1);
            achievementBadge = `<div class="scv-card-achievement-badge" title="${achUnlocked}/${achTotal} 成就">
                <svg viewBox="0 0 24 24"><path d="M12 15l-2-5 9-11h-5l2-5-7 5z"/></svg>
    ...
    function exportJSON() {
        const data = filteredGames.map(g => ({
            appid: g.appid,
            name: g.name,
            playtime_forever_minutes: g.playtime_forever || 0,
            playtime_forever_hours: parseFloat(g.playtime_hours || '0'),
            playtime_2weeks_minutes: g.playtime_2weeks || 0,
            playtime_2weeks_hours: parseFloat(g.playtime_2weeks_hours || '0'),
            last_played_timestamp: g.rtime_last_played || 0,
        ...
        showToast(`已导出 ${data.length} 款游戏为JSON 文件`, 'success');
    }
    ...
    function exportCSV() {
        ...
        showToast(`已导出 ${rows.length} 款游戏为CSV 文件`, 'success');
    }
    ...
    function toggleCardView() {
        ...
        const originalList = document.querySelector('[class*="FbG-gxCxUQw"]') || document.querySelector('[class*="ISUc4nhzLMI"]');
        if (!cardViewVisible) {
            ...
            if (toggleBtn) {
                toggleBtn.innerHTML = `<svg viewBox="0 0 16 16" width="16" height="16"><rect x="1" y="1" width="14" height="3" rx="1"/><rect x="1" y="6" width="14" height="3" rx="1"/><rect x="1" y="11" width="14" height="3" rx="1"/></svg><span>列表视图</span>`;
            }
        } else {
            ...
            const toggleBtn = document.getElementById('scv-toggle');
            if (toggleBtn) {
                toggleBtn.innerHTML = `<svg viewBox="0 0 16 16" width="16" height="16"><rect x="1" y="1" width="6" height="6" rx="1"/><rect x="9" y="1" width="6" height="6" rx="1"/><rect x="1" y="9" width="6" height="6" rx="1"/><rect x="9" y="9" width="6" height="6" rx="1"/></svg><span>卡片视图</span>`;
            }
        }
    }
    ...
    async function reloadGames() {
        ...
        updateLoadingProgress('正在滚动加载全部游戏...');
        await autoLoadAllGamesFromDOM();
        updateLoadingProgress('正在提取游戏数据...');
        let games = [];
        const rqGames = extractGamesFromReactQuery();
        if (rqGames && rqGames.length > 0) {
            games = enrichGameData(rqGames);
            console.log(`[Steam卡片] reloadGames: React Query 提取到 ${games.length} 个游戏`);
        } else {
            const ssrGames = extractGamesFromSSR();
            if (ssrGames && ssrGames.length > 0) {
                games = enrichGameData(ssrGames);
                console.log(`[Steam卡片] reloadGames: SSR 提取到 ${games.length} 个游戏`);
            } else {
                games = extractGamesFromDOM();
                console.log(`[Steam卡片] reloadGames: DOM 提取到 ${games.length} 个游戏`);
            }
        }
        removeLoadingOverlay();
        if (games.length > 0) {
    ...
    async function init() {
        console.log('[Steam卡片] 正在初始化 v3.1');
        ...
        if (!isPageReady()) {
            ...
            await new Promise((resolve) => {
                ...
                const poll = setInterval(() => {
                    attempts++;
                    if (isPageReady() || attempts >= maxAttempts) {
                        clearInterval(poll);
                        resolve();
                    }
    ...
    async function startApp() {
        ...
        if (needAutoLoad) {
            ...
            await autoLoadAllGamesFromDOM();
            updateLoadingProgress('正在提取游戏数据...');
            const rqGamesAfter = extractGamesFromReactQuery();
            if (rqGamesAfter && rqGamesAfter.length > 0) {
                games = enrichGameData(rqGamesAfter);
                console.log(`[Steam卡片] 滚动后从React Query提取到 ${games.length} 个游戏`);
            } else {
                const ssrGamesAfter = extractGamesFromSSR();
                if (ssrGamesAfter && ssrGamesAfter.length > 0) {
                    games = enrichGameData(ssrGamesAfter);
                    console.log(`[Steam卡片] 滚动后从SSR提取到 ${games.length} 个游戏`);
                } else {
                    games = extractGamesFromDOM();
                    console.log(`[Steam卡片] 回退从DOM提取到 ${games.length} 个游戏`);
                }
            }
            removeLoadingOverlay();
        } else {
        ...
        try {
            ...
            GM_registerMenuCommand('导出JSON', exportJSON);
